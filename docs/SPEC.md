...
| `circuit_e11` | CM = K × I × L / E; ampacity × bundling factor; the larger wins; fuse ≥ load × k and ≤ conductor; fittings from the owner's catalogs | system_voltage (any), circuit_type (12 kinds of circuit, each mapped to a load profile and reminder tags), current, length (the whole run, there and back) + unit, max_drop_percent (3/10/other + other), insulation_rating_c (105), engine_space, bundled + bundled_conductors, stud_size (optional, text), short_circuit_a (optional), manufacturer_fuse (optional), and the answer inputs own_ampacity_a, own_bundling_factor, own_k, own_short_circuit_a, own_cable_od_mm, own_heat_shrink_size, own_lug_part, own_crimp_die | Conductor: size_awg, size_mm2, cm_required, voltage_drop_size, printed_table_size, ampacity_size, drop_at_size; Protection: fuse_a, fuse_characteristic, interrupting, load_type; Fittings: cable_od, heat_shrink, lug, crimp_die; Bill of materials: bom_cable, bom_lugs, bom_heat_shrink, bom_fuse; plus `reminders` | see §12.6; no Excel formulas (the sheet exports app-computed values) |
