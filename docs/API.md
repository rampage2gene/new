...
A result's `value` may be `null`: a blank, never a guess. Its `note` says why and `CalcResult.asks` lists `{field, reason, input_key, unit, prompt, kind}` for each blank, `input_key` naming the input that answers it (`null` when the fix is elsewhere, such as the reference tables) and `kind` (`number` or `text`) what the box takes. Results may carry a `group` ("Conductor", "Protection", "Fittings", "Bill of materials") and the result a `reminders` list of `{topic, rule, clause, page, applies_to, status}` from the owner's E-11 cheat sheet.
...
`circuit_e11` ("Circuit: conductor and protection (ABYC E-11)") computes only from the owner's confirmed reference tables (below). Its `length` is the whole run, there and back; `circuit_type` (one of `battery_main`, `inverter`, `charger`, `alternator`, `dc_dc`, `solar`, `windlass`, `starter`, `bilge_pump`, `lights`, `electronics`, `general_dc`) sets the load profile and which reminders apply; `stud_size` (text, optional) picks the lug. Its results come in four groups plus `reminders`; a blank (`value: null`) carries an ask whose `input_key` is one of `own_ampacity_a`, `own_bundling_factor`, `own_k`, `own_short_circuit_a`, `own_cable_od_mm` (numbers), `own_heat_shrink_size`, `own_lug_part`, `own_crimp_die` (text), or `stud_size` itself when no stud was given - send that input with the person's value and run again; the answered part is then noted "entered by you". With no confirmed table at all it returns one blank whose ask has `"field": "reference"`.
...
| `PUT /reference/e11/tables/{id}` | the table file with `status` `draft` or `confirmed` | the saved table; 422 with the loader's reason (no page, a row without a page, a fixture, a fuse class without its datasheet, a catalog table without its unit or a tubing size that does not shrink); 404 for an unknown id. The ids are the eight tables of the standard plus the three catalog tables `cable_dimensions`, `heat_shrink` and `lugs` (typed from the owner's catalogs; `source.document` names the catalog, pages optional) |
| `DELETE /reference/e11/tables/{id}` | — | the table as it now is (the bundled copy, or missing) |
| `POST /reference/e11/tables/{id}/import` | `{"document_id", "page", "table_index": 0}` | a draft copied from the table the app detected there: cells whose row label and column header fit the layout are copied, the rest stay blank and `edits` marks what to check; 404 when no table was detected on that page; 422 for `fuse_classes` and the three catalog tables (typed from datasheets and catalogs, never imported) |
| `POST /reference/e11/import-all` | `{"document_id"}` | `{copied: [{id, page}], not_found: [ids], ambiguous: [{id, pages}], kept: [ids], unfit: [{id, page, reason}]}`. Every table of the standard this document appears to hold, copied in one go as drafts to check and confirm. A table already saved as the person's own is `kept`, never overwritten; a table the words on no page match is `not_found` (`constants` always is - K and the formula are printed as text, not as a table); two or more pages that match is `ambiguous`, for the person to choose in that table's own import row; a page that matched but does not read as that layout is `unfit` with the loader's reason. 404 for an unknown document |
